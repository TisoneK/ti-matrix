...
>   `https://github.com/TisoneK/ti-matrix.git`, read `.context_ledger/kickoff.md`, follow it."* If the
...
- **Project name:** ti-matrix
- **Project repository URL:** https://github.com/TisoneK/ti-matrix.git
...
- **Cloud/sandbox agent** — clone the project repo only:
  ...
  ```bash
  # Public repo — no PAT needed for the clone; pushes use the session's
  # own credentials (never a PAT in chat for this project).
  git clone https://github.com/TisoneK/ti-matrix.git ti-matrix && cd ti-matrix
  git remote set-url origin https://github.com/TisoneK/ti-matrix.git
