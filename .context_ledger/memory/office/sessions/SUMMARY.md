...
---
- **2026-09-26 — Session 9** — Iris / deepseek-flash — three pieces: a settlement diagnosed as unweighed (flagged, not fixed); the API key now remembered across launches, encrypted and endpoint-bound (`1ed4c01`); and a filesystem search that hung the whole app, bounded and made non-blocking (`82796d0`).
  Key discovery: a fan that claims `done` ends the run (`ti_matrix/search.py:363`), so a competing candidate is never generated — and the guard `DESIGN.md` describes as "`done` requires an answer grounded in facts" checks only that the answer is non-empty, proved by running the engine with an answer that appears in no observation. Second: the hang was not a slow search but a probe running *on the host's event loop*, which is why the window looked alive and Stop could not answer.
