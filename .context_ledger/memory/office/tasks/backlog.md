...
| B-2026-09-26-10 | **A challenge can no longer mistake re-confirmation for competition (`68e2222`) — nothing yet forces a run to ever generate a second real candidate in the first place.** B-2026-09-26-8's fix closed direction (b) of the two the row named: `_challenge` now excludes any action sharing the claim's own `path` before probing it (`_subject`), so a `list_dir` of the exact folder `find_files` already claimed no longer counts as something surviving a challenge — it is the same candidate confirmed a second way, not an alternative. What this does **not** do, stated plainly: it does not make the proposer generate a genuinely different candidate to begin with. A run whose search never considers a sibling directory at all still settles the moment its one candidate clears the direct-look guard and finds nothing else to exclude — the challenge has nothing to compare against because nothing else was ever proposed. Direction (a) from the original row — the evaluator prompt asking explicitly for a distinguishing structural comparison when candidates share surface features — is still unscoped, and overlaps `B-2026-09-26-9`'s parallel-branch question: both are really "how much breadth does a search owe a goal before it may settle," one at the prompt level and one at the architecture level. Not fixed here. |
| B-2026-09-26-9 | **Scope real parallel-branch search — a supervisor request, not yet a design.** `StateEngine` is, by its own docstring, "a beam of one with backtracking": exactly one state is ever live, and a fan's several candidates are next-*moves* scored together with only the best applied — never several live states explored and compared simultaneously. The supervisor watched a run and wanted to see multiple branches expanding in real time, comparing states against each other as the run went, not a linear main-line with small dashed "passed over" lanes beside it (which the current tree already draws, folded by default behind "fold cold branches"). This is a different search algorithm — true beam width > 1, or a broader tree search — not a UI change, and it changes the engine's budget accounting, its event vocabulary (today's `selected`/`backtrack` assume one current state), the renderer's tree layout, and possibly the challenge round (B-2026-09-26-5) built on the single-state assumption. To do: a design brief scoping cost, event/protocol changes, and a minimal slice to prototype against, before any engine code — this is exactly the kind of change B-2026-09-23-12's own note already flagged ("if the search's shape is ever wanted at proposal time... that needs the history passed to the proposer or a `Retriever` port, and it is a protocol change that should land before B-2026-09-23-11 opens worlds to users"). **The supervisor's own sketch of the shape, given after `find_files`' breadth-first fix (`1164feb`) surfaced every name match in one search:** the first round of a locate-style goal should fan out across *every* name match at once — one tree node per candidate, not one node for whichever the proposer happened to try first — each gathering only cheap, fast evidence (a `stat_path`, a shallow `list_dir`; nothing expensive yet), with all of it visible in the tree side by side *before* the search commits to diving deeper into any single one. Scoring then has real comparative evidence behind it (this candidate has a `.git` with three years of commits and a `node_modules`; that one is three files and a stray backup) instead of the first candidate's evidence taken alone. This is a genuine design direction for the brief above, not itself scoped — it still needs the same protocol/budget/renderer questions answered (what does "cheap evidence" mean generically across worlds that are not filesystems; how many parallel nodes at once; how does budget accounting change when a round is N probes wide instead of one fan deep). Not started. |
