...
| B-2026-09-23-8 | **Draw the branch points in the tree.** The ledger half shipped in `9daf9a4` — every row now shows what the decision passed over, with each candidate's score and the world's refusals. The tree still draws a spine: `readTree` builds nodes only from `state` events, so the rejected siblings have no marker there. Add a stub per rejected candidate off the node it was weighed at (leaves, never subtrees — each was probed exactly once), and the count in the tree header. Still pairs with B-2026-09-23-9: the maze branches 1.35 ways per decision, so the stubs are sparse until a world actually branches. Brief: `.context_ledger/memory/office/plans/the-roads-not-taken-brief.md`. |
