...
| B-2026-09-26-2 | **`find_files` walks its whole root twice, and the root now defaults to home.** `ti_matrix/adapters/files.py` runs `root.rglob("*")` once to collect hits and a second time to count how much was searched — two unbounded recursive walks of whatever root the run was given. That was survivable while a root had to be typed in deliberately; now that the files world defaults to the home directory, one `find_files` can walk an entire user profile (caches, `node_modules`, `AppData`). Observed: a builtin run rooted at home did not finish in 90s. To do: bound the walk (depth and/or entry cap, skipping the usual noise directories), collapse the two walks into one count, and have the action say when the cap was hit so a run can tell "nothing matched" from "I stopped looking". |
