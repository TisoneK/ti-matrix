...
| B-2026-09-23-12 | **Let the seats ask for what they need instead of being handed everything.** `597b12d` fixed the immediate bug — the proposer was shown `facts[-8:]`, costing 60% of a maze run's probes to re-learning known places — but "show everything" is the absence of selection, not selection, and it stops working at the scale this product is heading for. The rule-based seats already select (`MazeKnowledge` computes hop distances and proposes the nearest frontier; `FilesReasoner` ranks by goal words and path depth); the model seat cannot, because `propose(state, n, avoid)` hands it a flat tuple of strings with no node id, no parent and no structure — the tree is stamped onto *events*, so it is output, not state. Two designs: an optional `Retriever` port beside Simulator/Confirmer, or `recall` as an action the model proposes, which puts what it chose to look up in the ledger as evidence. **Decide before B-2026-09-23-6 is written**, not after. Brief: `.context_ledger/memory/office/plans/recall-what-is-relevant-brief.md`. |
