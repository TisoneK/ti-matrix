"""Play with the engine driving a repository's Context Ledger — in a browser, from one file.

    python examples/ledger_ui.py --vault ~/code/my-project
    # then open http://127.0.0.1:8765

Ask it what a project already knows: what is in flight, what was decided and why, which approach already
failed, what friction the last sessions hit. Nine read-only actions answer from the vault, `recall` adds what
this session has learned about the world, and the run's own results can be written back when you tick the box —
the box says which two files it would touch, and nothing is written before you tick it.

Nothing here needs the context-ledger project installed: a vault is files, and this reads the formats that
project's schema defines. What it will not do is invent one. If the vault is missing its session notes or its
Findings table, the run says so and writes nothing, rather than creating a file the schema did not ask for.
"""
from __future__ import annotations

from ti_matrix.adapters.context_ledger import LedgerEnvironment, LedgerRecorder, LedgerVault
from ti_matrix.tools import EngineTools

TITLE = "Ti Matrix — playing with a Context Ledger"
ENV_NOTE = ("Nine read-only actions over a project's memory — the orientation digest, the work queue, the "
            "decisions in force, the session registry, the friction logs, and a search across all of it — plus "
            "`recall`, which adds what this session has learned. Every action reads, so a run cannot change the "
            "vault while it thinks. Tick the box to append its results afterwards.")
FIELDS = [
    {"name": "vault", "label": "Project holding .context_ledger/", "default": ".",
     "placeholder": "~/code/my-project", "group": "env"},
    {"name": "record", "label": "Record this run back into the vault — appends to the office's session notes "
                                "and promotes its facts into the parking lot's Findings",
     "default": "off", "type": "checkbox", "group": "env"},
]


def build_environment(config: dict[str, str], stats) -> EngineTools:
    """This example's world: a project's memory, and the engine's own recall of this session."""
    env = LedgerEnvironment(config.get("vault") or ".")
    if not env.vault.exists():
        raise RuntimeError(f"no Context Ledger at {env.vault} — bootstrap one in that project first, or point "
                           f"--vault at a project that has .context_ledger/")
    return EngineTools(env, memory=stats)


def epilogue(play, env, config: dict[str, str]) -> None:
    """What this example does that the filesystem one does not: offer to write the run back."""
    vault = LedgerVault(config.get("vault") or ".")
    if config.get("record") != "on":
        play.note("not recorded — tick the box to append this run into the vault")
        return
    recorder = LedgerRecorder(vault, model=config.get("model", ""))
    for event in play.raw:  # the engine's own events, exactly as it emitted them
        recorder.observe(event)
    play.note(*recorder.record(goal=config.get("goal", "")).to_text().splitlines())
    findings = [r.ident for r in vault.parking_lot() if r.section == "Findings"]
    play.note(f"the vault now holds {len(findings)} finding(s): {', '.join(findings[-3:]) or 'none'}")
